...
switch (command) {
	...
	case "import": {
		...
		if (!filePath) {
			...
			try {
				...
				console.error(
					"Usage: pi-antigravity-rotator import <path-to-json-file>",
				);
				process.exit(1);
			}
		}
	...
	case "keys":
		{
			...
			if (!isDbConfigured()) {
				console.error(
					"Virtual keys require PostgreSQL. Set PI_ROTATOR_DATABASE_URL.",
				);
				process.exit(1);
			}
			await initDb();
			...
			const { listVirtualKeys, generateVirtualKey, deleteVirtualKey } =
			...
			if (subAction === "list") {
				...
				if (keys.length === 0) {
					console.log(
						"No virtual keys found. Use 'pi-antigravity-rotator keys generate' to create one.",
					);
				} else {
					...
					for (const k of keys) {
						...
						console.log(
							`  Models:    ${k.models && k.models.length > 0 ? k.models.join(", ") : "(all)"}`,
						);
	...
	default:
		...
		console.log(
			"  pi-antigravity-rotator import    Import accounts from JSON file (default: ~/.config/antigravity/accounts.json)",
		);
		console.log(
